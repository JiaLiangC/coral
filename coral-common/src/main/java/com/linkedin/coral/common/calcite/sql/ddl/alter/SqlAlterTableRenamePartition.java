package com.linkedin.coral.common.calcite.sql.ddl.alter;
import java.util.List;
import com.google.common.collect.ImmutableList;
import org.apache.calcite.sql.*;
import org.apache.calcite.sql.parser.SqlParserPos;

public class SqlAlterTableRenamePartition extends SqlCall {
    private final SqlNode oldPartitionSpec;
    private final SqlNode newPartitionSpec;

    public SqlAlterTableRenamePartition(SqlParserPos pos, SqlIdentifier tableName, SqlNode oldPartitionSpec, SqlNode newPartitionSpec) {
        super(pos);
        this.oldPartitionSpec = oldPartitionSpec;
        this.newPartitionSpec = newPartitionSpec;
    }

    @Override
    public SqlOperator getOperator() {
        return null;
    }

    @Override
    public List<SqlNode> getOperandList() {
        return ImmutableList.of();
    }

}